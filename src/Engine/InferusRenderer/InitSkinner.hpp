/**
 *                      --- Initialization Skinner ---
 *
 * The sole purpose of this file is to make my vulkan context initialization cleaner
 *
**/

#pragma once

#define INIT_FUNC static inline InferusResult
#define PICK_FUNC static inline bool
...
namespace InitSkinner {
    ...
    namespace Instance {
        ...
        std::array<const char*, 1> VALIDATION_LAYERS_EXTENSION = { VK_EXT_DEBUG_UTILS_EXTENSION_NAME };
#endif

        INIT_FUNC CreateInstance(Window& Window, VkInstance &Instance) {
    ...
    namespace PhysicalDevice {
        ...
        INIT_FUNC PickPhysicalDevice(VkInstance Instance, VkPhysicalDevice &PhysicalDevice) {
                ...
                for (VkPhysicalDevice CurrentPhysicalDevice : PhysicalDevices) {
                    ...
                    bool ExtensionFound;
                    for (const char* Extension : WideContextVars::DEVICE_EXTENSIONS) {
    ...
    namespace LogicalDevice {
        INIT_FUNC CreateLogicalDevice(
                VkPhysicalDevice PhysicalDevice,
                std::vector<QueueContext*> AllQueues,
                VkDevice &Device)
        {
            ...
            DeviceCreateInfo.enabledExtensionCount = static_cast<uint32_t>(WideContextVars::DEVICE_EXTENSIONS.size());
            DeviceCreateInfo.ppEnabledExtensionNames = WideContextVars::DEVICE_EXTENSIONS.data();
    ...
    namespace SurfaceOptions {
        PICK_FUNC PickSurfaceFormat(
                VkPhysicalDevice PhysicalDevice,
                VkSurfaceKHR Surface,
                ...
                VkSurfaceFormatKHR &PickedSurfaceFormat)
        {
            ...
            for (VkSurfaceFormatKHR CurrSurfaceFormat : SurfaceFormats) {
                if ((CurrSurfaceFormat.format == DesirableSurfaceFormat.format) &&
                    ...
                    PickedSurfaceFormat = CurrSurfaceFormat;
                    return true;
                }
            }
            ...
            PICK_FUNC PickPresentMode(
                VkPhysicalDevice PhysicalDevice,
                VkSurfaceKHR Surface,
                std::vector<VkPresentModeKHR> PresentModesTierList,
                VkPresentModeKHR &PickedPresentMode)
        {
            ...
            for (VkPresentModeKHR ComparingPresentMode : PresentModesTierList ) {
                for (VkPresentModeKHR CurrPresentMode : PresentModes) {
                    if (ComparingPresentMode == CurrPresentMode) {
                        PickedPresentMode = CurrPresentMode;
                        return true;
                    }
                }
            }
            // Non desirable present mode picked
            PickedPresentMode = PresentModes[0];
            return false;
        }
    };
};
